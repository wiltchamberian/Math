...
//#define ENTRIES(A,B,C,D) (entries[A]*entries[B]-entries[C]*entries[D])
    ...
    Matrix4x4 Matrix4x4::getInverseTransform() const
    {
        ...
        m1 = m1.getInverse();
        Vector3D v = m1 * Vector3D(m2.entries[12], m2.entries[13], m2.entries[14]);
    ...
    Matrix4x4 Matrix4x4::getScale(const Vector3D& scaleFactor)
    {
        Matrix4x4 m;
    ...
    Matrix4x4 Matrix4x4::getRotationAxis(const double radians, const Vector3D& axis)
    {
        Matrix4x4 m;
    ...
    {
        loadIdentity();
        entries[0] = static_cast<scalar>(width) / 2.0f;
        entries[5] = -static_cast<scalar>(height) / 2.0f;
        entries[3] = static_cast<scalar>(left) + static_cast<scalar>(width) / 2.0f;
        entries[7] = static_cast<scalar>(top) + static_cast<scalar>(height) / 2.0f;
    }

    void Matrix4x4::decomposeTransRotScale(Vector3D& position, Quaternion& rotation, Vector3D& scale) const
    {
