...
#define Z Polynomial3D({{0,0,1,1}}); 
    ...
    public:
        ...
        float value(const Vector3D& v) const;
        void merge();
        Vector3D gradient(float x,float y,float z) const;
        Vector3D gradient(const Vector3D& v) const;       
        //形式微分 df/dx;
        ...
        float partialX(const Vector3D& p) const;
        float partialY(const Vector3D& p) const;
        float partialZ(const Vector3D& p) const;
        void sort();
        float root(float t, const Vector3D& origin, const Vector3D& dir, int maxIterTimes = 10, float tol=0.001);
    ...
    float polyRoot(const Polynomial3D& poly, const Polynomial3D& partialX ,const Polynomial3D& partialY, const Polynomial3D& partialZ
        ,float t, const Vector3D& origin, const Vector3D& dir, int maxIterTimes = 10, float tol = 0.001);
}
