...
namespace Sun {
    ...
    float Polynomial3D::value(const Vector3D& v) const {
    ...
    Vector3D Polynomial3D::gradient(float x, float y, float z) const {
        Vector3D res;
    ...
    Vector3D Polynomial3D::gradient(const Vector3D& v) const {
    ...
    float Polynomial3D::partialX(const Vector3D& p) const {
        return partialX(p.x, p.y, p.z);
    }

    float Polynomial3D::partialY(const Vector3D& p) const {
        return partialY(p.x, p.y, p.z);
    }

    float Polynomial3D::partialZ(const Vector3D& p) const {
    ...
    float Polynomial3D::root(float t, const Vector3D& origin, const Vector3D& dir, int maxIterTimes, float tol) {
    ...
    Polynomial3D Polynomial3D::operator* (float c) const {
        if (equal(c, 0.f))return Polynomial3D::zero();
        Polynomial3D res = *this;
    ...
    float polyRoot(const Polynomial3D& poly, const Polynomial3D& partialX, const Polynomial3D& partialY, const Polynomial3D& partialZ,
        float t, const Vector3D& origin, const Vector3D& dir, int maxIterTimes, float tol) {
        Vector3D p = origin + dir * t;
        float r = poly.value(p);
        if (equal(r, 0.0)) return t;
